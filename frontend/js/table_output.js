...
async function renderCustomerPage() {
  ...
  for (const subtype of subtypes) {
    if (cachedData[subtype]) {
      ...
      ids.forEach(id => { typeMap[id] = subtype; });
    }
  }
...
async function renderEmployeePage() {
  ...
  tableDiv.innerHTML += await buildRelationSection("Customer_Transaction", true, false);
}
...
async function renderManagerPage() {
  ...
  tableDiv.innerHTML += await buildRelationSection("Individual_Order", true, true);
  ...
  tableDiv.innerHTML += await buildRelationSection("Key_Order_Times", false, false);
}
...
function buildTransactionSummaryJoin(allowEdit) {
  ...
  const mergedRows = txData["data"].map(txRow => {
    const txId   = txRow[0];
    const sumRow = sumMap[txId] ? sumMap[txId].slice(1) : sumExtraCols.map(() => "N/A");
    return [...txRow, ...sumRow];
  });

  const canEdit   = allowEdit && cachedPrivileges["Customer_Transaction"]?.includes("UPDATE");
  const canDelete = allowEdit && cachedPrivileges["Customer_Transaction"]?.includes("DELETE");

  return buildTableHTML("Customer_Transaction + Complete_Order_Summary", mergedCols, mergedRows, canEdit, canDelete);
}
...
async function buildRelationSection(relation, allowEdit, allowInsert) {
  ...
  const canInsert = allowInsert && cachedPrivileges[relation]?.includes("INSERT");
...
function buildTableHTML(label, cols, rows, canEdit = false, canDelete = false) {
  const editHeader = (canEdit || canDelete)
...
async function getPrivileges(role_name) {
  const response = await fetch(`${API}/${role_name}/privileges`, { method: "GET" });
...
function renderOrder() {
  ...
  orderDiv.innerHTML = window.currentOrder.map((item, index) => {
    total += item.price;
